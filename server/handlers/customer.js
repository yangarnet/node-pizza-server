...
const customer_handler = {
   ...
   register: (data, callback) => {
        if (helper.isValidCustomerData(data.payload)) {
            const { firstname, lastname, email, password, address } = data.payload;
            ...
            _data.read('users', username, (err, res) => {
                if (err) {
                    ...
                    _data.create('users', username, newUser, (err, response) => {
                        if (!err && response) {
                            callback(200, {success: 'add new user success'});
                        } else {
                            callback(500, {error: 'fail to add new user'});
                        }
                    });
                } else {
                    ...
                    if (inputPassword === res.password && email === res.email) {
                        ...
                        token_handler.post(data, err => {
                            if (err === 200 ) {
                                ...
                                callback(500, {error: 'create user token error after log in'});
                            }
                        });
                    } else {
                        callback(403, {error: 'invalid password or email address'});
                    }
                } else {
                    callback(err, res);
                }
    ...
    */
    ...
    /*
        ...
        @payload: { "email":"yangarnet@gmail.com", "password":"passw0rd", "address":"102 william st"}
    */
   update: (data, callback) => {
        ...
        token_handler.verifyToken(token, email, isValid => {
            if (isValid) {
                ...
                _data.read('users', username, (err, userData) => {
                    ...
                    _data.update('users', username, userData, (err, res) => {
                        if (!err && res) {
                            callback(200, {success: 'user data update success'});
                        } else {
                            callback(500, {error: 'user data update failed'});
                        }
                    });
                    } else {
    ...
    /*
        ...
        @payload: {"email":"yangarnet@gmail.com"}
    */
    cancel: (data, callback) => {
        ...
        if (token && email) {
            token_handler.verifyToken(token, email, isValid => {
                if (isValid) {
                    _data.delete('users',email.slice(0, email.indexOf('@')), (err, response) => {
                        if (!err && response) {
                            // delete user tokens
    ...
    /*
        ...
        @method: GET
        @headers: {token:'asdf', email:'your_email@gmail.com'}
    */
    listMenu: (data, callback) => {
        ...
        if (token && email) {
            token_handler.verifyToken(token, email, isValid => {
                if (isValid) {
                    const pizzaMenus = menu.restaurant.menu.filter(m => m.category === 'Pizza')[0];
    ...
    /*
        ...
        @method: POST
        @header: { token, email }
        @payload: { pizza, number, unitprice}
    */
    add: (data, callback) => {
                ...
                if (isValid) {
                    ...
                    _data.read('cart', email, (err, pizzaData) => {
                        if (err) {
                            const order = {
                                items: [{
                                    pizza, number, unitprice
                                }]
                            }
                            _data.create('cart', email, order, (err, res) => {
                                if (!err && res) {
                                    callback(200, {success: 'add new pizza order success'});
                                } else {
                                    callback(500, {error: 'add new pizza order failed'});
                                }
                            });
                        } else {
                            // add new order to existing
                            pizzaData.items.push({pizza, number, unitprice});
                            _data.update('cart', email, pizzaData, (err, response) => {
                                if (!err && response) {
                                    callback(500, {success: 'update pizza order success'});
                                } else {
                                    callback(500, {error: 'update pizza order failed'})
                                }
                            });
                        }
                    });
